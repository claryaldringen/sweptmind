...
describe("formatLocationName", () => {
  ...
  it("formats name + country when state is missing", () => {
    expect(
      formatLocationName({ name: "Praha", country: "Česko" })
    ).toBe("Praha, Česko");
  });

  it("formats name + country for foreign cities", () => {
    expect(
      formatLocationName({ name: "Berlín", country: "Německo" })
    ).toBe("Berlín, Německo");
  });

  it("formats name + state + country for foreign places with state", () => {
    expect(
      formatLocationName({ name: "Bergen", state: "Vestland", country: "Norsko" })
    ).toBe("Bergen, Vestland, Norsko");
  });

  it("returns just name when country is missing", () => {
    expect(
      formatLocationName({ name: "Somewhere" })
    ).toBe("Somewhere");
  });
